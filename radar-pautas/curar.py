...
def montar_prompt(config, itens):
    ...
    temas = "\n".join(f"- {t}" for t in config["temas_de_interesse"])
    ...
    instrucao = f"""Você é um curador de notícias internacionais sobre IA, narrativa e educação. Sua tarefa é triar itens brutos e selecionar os mais relevantes para o público brasileiro, redigindo ganchos de pauta impessoais que um jornalista possa usar diretamente.
    ...
    - gancho_pauta: um parágrafo de sugestão de pauta em português-padrão, sem clichês de texto gerado por IA e sem travessões, explicando por que o fato rende matéria e que tipo de especialista poderia comentar, sem mencionar nenhuma pessoa específica
